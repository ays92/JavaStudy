...
public class Array1Ref1 {
    public static void main(String[] args) {
        ...
        //변수 값 사용
        System.out.println("학생 0점수 : " + students[0]);
        System.out.println("학생 1점수 : " + students[1]);
        System.out.println("학생 2점수 : " + students[2]);
        System.out.println("학생 3점수 : " + students[3]);
        System.out.println("학생 4점수 : " + students[4]);
    }
}
        /* 중요개념 1
        ...
        int[] students 와 같은 배열 변수에는 "배열"을 담을 수 있다.
        ...
        */
        ...
        new 는 "새로 생성한다는 뜻"이고, int[5] 는 int 형 변수 5개라는 뜻이다.
        따라서 int 형 변수 5개를 다룰 수있는 배열을 새로 만든다는 뜻이다.
        앞서 int student1 , int student2 ... int student5 까지 총 5개의 변수를 직접 선언했다.
        배열을 사용하면 이런 과정을 한번에 깔끔하게 처리할 수 있다.

        배열과 초기화
        new int[5] 라고 하면 총 5개의 int 형 변수가 만들어진다.
        "자바는 배열을 생성할 때 그 내부값을 자동으로 초기화한다".
        숫자는 0 , boolean 은 false , String 은 null로 초기화 된다.
         */
     ...
     *   쉽게 이야기해서 참조값을 통해 메모리에 있는 실제 배열에 접근하고 사용할 수 있다.
     ...
     *   앞서 int[] students 변수에 참조값( x001 )을 보관해두었다. 이 변수를 통해서 이 배열에 접근할 수 있다.
        ...
        참조값에 대한 더 자세한 내용은 뒤에서 다룬다.
        지금은 생성한 배열을 참조할 수 있는, 메모리의 주소를 나타내는 특별한 값이 있다는 정도만 이해하면 충분하다
         */
        ...
        예를 들어서 Scanner 를 사용해서 사용자 입력에 따라 size변수의 값이 변하고, 생성되는 배열의 크기도 달라질 수 있다.
        이런 것을 동적 메모리 할당이라 한다. 기본형은 선언과 동시에 사이즈가 정적으로 정해지지만,
        참조형을 사용하면 이처럼 동적으로 크기가 변해서 유연성을 제공할수 있다.
        기본형은 사용할 값을 직접 저장한다. 반면에 참조형은 메모리에 저장된 배열이나 객체의 참조를 저장한다.
        이로인해 참조형은 더 복잡한 데이터 구조를 만들고 관리할 수 있다. 반면 기본형은 더 빠르고 메모리를 효율적으로 처리한다.
